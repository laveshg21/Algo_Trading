...
#DEFINING WEIGHT FUNCTION:
    ...
    def weight(k,W1,W2,W4,W8,W16,M6,Y1,Y2):
        oplo_W1 = pd.DataFrame(columns=['Lord'])
        oplo_W1['Lord'] = (((W1['Open']-W1['Low'])*100 )/W1['Open'])
        true_W1 = oplo_W1[oplo_W1.Lord > k]
        freq_W1 = (true_W1.size*100)/(W1.size)

        oplo_W2 = pd.DataFrame(columns=['Lord'])
        oplo_W2['Lord'] = (((W2['Open']-W2['Low'])*100 )/W2['Open'])
        true_W2 = oplo_W2[oplo_W2.Lord > k]
        freq_W2 = (true_W2.size*100)/(W2.size)
    
        oplo_W4 = pd.DataFrame(columns=['Lord'])
        oplo_W4['Lord'] = (((W4['Open']-W4['Low'])*100 )/W4['Open'])
        true_W4 = oplo_W4[oplo_W4.Lord > k]
        freq_W4 = (true_W4.size*100)/(W4.size)
    
        oplo_W8 = pd.DataFrame(columns=['Lord'])
        oplo_W8['Lord'] = (((W8['Open']-W8['Low'])*100 )/W8['Open'])
        true_W8=oplo_W8[oplo_W8.Lord > k]
        freq_W8 = (true_W8.size*100)/(W8.size)

        oplo_W16 = pd.DataFrame(columns=['Lord'])
        oplo_W16['Lord'] = (((W16['Open']-W16['Low'])*100 )/W16['Open'])
        true_W16=oplo_W16[oplo_W16.Lord > k]
        freq_W16 = (true_W16.size*100)/(W16.size)

        oplo_M6 = pd.DataFrame(columns=['Lord'])
        oplo_M6['Lord'] = (((M6['Open']-M6['Low'])*100 )/M6['Open'])
        true_M6=oplo_M6[oplo_M6.Lord > k]
        freq_M6 = (true_M6.size*100)/(M6.size)

        oplo_Y1 = pd.DataFrame(columns=['Lord'])
        oplo_Y1['Lord'] = (((Y1['Open']-Y1['Low'])*100 )/Y1['Open'])
        true_Y1=oplo_Y1[oplo_Y1.Lord > k]
        freq_Y1 = (true_Y1.size*100)/(Y1.size)

        oplo_Y2 = pd.DataFrame(columns=['Lord'])
        oplo_Y2['Lord'] = (((Y2['Open']-Y2['Low'])*100 )/Y2['Open'])
        true_Y2=oplo_Y2[oplo_Y2.Lord > k]
